    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gradient-to-r from-amber-800 to-amber-950 p-6 rounded-xl shadow-lg gap-4">
            <div>
                ...
                <p class="text-amber-200/70 text-xs mt-1">Ingresa los datos del nuevo cliente.</p>
            </div>
            ...
            <div class="bg-white overflow-hidden rounded-2xl border border-amber-100 shadow-md p-8">
                <form method="POST" action="{{ route('clientes.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="apellido" name="apellido" type="text" class="block mt-1 w-full" :value="old('apellido')" required />
                        <x-input-error :messages="$errors->get('apellido')" class="mt-2" />
                    </div>

                    <div class="mb-4">
                        <x-input-label for="telefono" value="Teléfono" />
                        <x-text-input id="telefono" name="telefono" type="text" class="block mt-1 w-full" :value="old('telefono')" />
